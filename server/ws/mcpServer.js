...
export function buildControlMcpServer(deps) {
  ...
  server.tool(
    'list_group_sessions',
    'List all sessions in this orchestration group (workers and orchestrator) with role, app, cwd, live status, autoYes (whether automatic permission-approval is enabled; null when the member has no live session, e.g. a restored one), idleForMs (ms since each session last produced output; null when no live session exists), and dockerAvailable/dockerReason (see get_tab_status -- whether THIS member can currently use docker). A member expected to be working but showing a large idleForMs may be stuck -- treat that as an anomaly signal only: if it piles up alongside repeated wait_for_handoff timeouts, confirm with a single read_output; otherwise keep waiting on wait_for_handoff instead of polling. Before handing a member a docker-dependent task, check its dockerAvailable here rather than finding out from a failure: a rootless dockerd can only serve ONE session per project at a time (the same project opened in two sandboxes only gives docker to whichever session\'s dockerd actually won the startup race, which is NOT necessarily workerA -- check, don\'t assume).',
    {},

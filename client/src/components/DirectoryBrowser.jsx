  ...
  useEffect(() => {
    authFetch('/api/dirs/home').then(r => r.json()).then(data => {
      ...
      if (data.availableApps) {
        ...
        const avail = ['claude', 'opencode', 'copilot', 'codex', 'commandcode'].filter((a) => data.availableApps[a]);
        ...
        const effectiveDefault = ['claude', 'opencode', 'copilot', 'codex', 'commandcode'].includes(data.defaultApp) && !localStorage.getItem(APP_KEY)
          ? data.defaultApp
          : appDefault;
    ...
    <>
      ...
      <p className="open-menu-note">
        {forceSandbox
          ...
          : `サンドボックス: 隣接プロジェクトを隔離し、内部に rootless docker を用意。GPG/ssh-agent/ツール導入は既定オフ、このディレクトリ (${displayPath(currentPath, homeDir)}) に記憶されます。`}
      </p>
    </>
  );
          ...
          <div className="resume-dialog open-dialog" onClick={(e) => e.stopPropagation()}>
            ...
            {launchMode === 'single' ? (
              <>
                ...
                <div
                  className={`open-menu-item${availableApps && !availableApps.commandcode ? ' open-menu-item-disabled' : ''}`}
                  onClick={() => chooseApp('commandcode')}
                  title={availableApps && !availableApps.commandcode ? 'サーバーに未インストール' : ''}
                >
                  <span className="open-menu-check">{appDefault === 'commandcode' ? '✓' : ''}</span>
                  Command Code
                </div>

...
- **rtk** (`rtk-ai/rtk`) — LLM 向け出力圧縮 CLI プロキシ。GitHub Releases から静的バイナリを取得し、`$HOME/.local/bin/rtk` に配置します (sandbox PATH の先頭なのでエージェントからそのまま `rtk` で使えます)。バージョンと sha256 をピン留めし、検証してから導入します。導入後は `rtk telemetry disable` で対話プロンプトを無効化した上で `rtk init -g --opencode --no-patch` を実行し、**opencode プラグイン** (`$HOME/.config/opencode/plugins/rtk.ts`) を配置するため、opencode セッション内の bash コマンド (git / pnpm / ls 等) が自動的に rtk 経由のコンパクト出力に書き換えられます (プロンプト待ちによるブロックを防ぐため、すべて非対話で実行。rtk バイナリとプラグインの両方が成功した場合のみマーカーを作成し、失敗時は次回起動で再試行)。
